...
import { type GradientWavesProps } from "./Types/GradientWavesTypes.ts";
import styles from "./GradientWaves.module.scss";
...
const GradientWaves = ({
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    const u = ctx.program.uniforms as GradientUniforms;
  ...
  const classes = [styles.container, unsupported ? styles.unsupported : null, className]
    .filter(Boolean)
    .join(" ");
